...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use mild seasonal adjustment to keep mock data realistic.
        ...
        current_temp = round(base_temp + random.uniform(-4, 4), 1)
        current_humidity = round(random.uniform(35, 70), 1)
        current_wind = round(random.uniform(3, 18), 1)

        hourly = []
        for day_idx in range(forecast_days):
            for hour in range(24):
                # Simple daylight curve: peak irradiance around noon.
                daylight_factor = max(0.0, 1.0 - abs(hour - 12) / 6.0)
                condition = random.choices(
                    ["sunny", "partly_cloudy", "cloudy"],
                    weights=[0.35, 0.4, 0.25],
                    k=1,
                )[0]
                cloud_factor = {"sunny": 1.0, "partly_cloudy": 0.7, "cloudy": 0.35}[condition]
                irradiance = round(1000 * daylight_factor * cloud_factor, 1)

                # Temperature rises midday and cools overnight.
                diurnal_temp = base_temp + 6 * daylight_factor + random.uniform(-2, 2) - day_idx * 0.2

                hourly.append({
                    "day": day_idx + 1,
                    "hour": hour,
                    "temperature_c": round(diurnal_temp, 1),
                    "condition": condition,
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for hour in range(24):
            ...
            # Add small hourly variability
            rate = round(base_rate * multiplier + random.uniform(-0.01, 0.01), 4)
            hourly_rates.append({
                "hour": hour,
                "rate": max(0.01, rate),
                "period": period,
